...
namespace LABA2
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private async void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                MessageBoxResult mBox = MessageBox.Show("Хоите загрузить данные с компьютера?", "???", MessageBoxButton.YesNo);
                if (MessageBoxResult.Yes == mBox)
                {
                    LoadedFiles();
                }
            }
        ...
        private async void Button_Click_1(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                DTgreed.ItemsSource = dt.DefaultView;
        ...
        private async void Button_Click_2(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                EXCEPTION1.Content = "Какое-то поле не заполненно!";
            }
        }
        ...
        private async void Button_Click_3(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    bool a = false;
                    SqlCommand command = new SqlCommand("SELECT [ИдентификаторУБИ] FROM InformationSecurityThreats1", sqlConnection);
                    SqlDataReader sqlReader = await command.ExecuteReaderAsync();
                    try
                    {
                        while (await sqlReader.ReadAsync())
                        {
                            if (Convert.ToString(sqlReader["ИдентификаторУБИ"]) == textBox16.Text)
                            {
                                a = true;
                            }
                        }
                    ...
                    if (a)
                    {
                        command = new SqlCommand("DELETE FROM InformationSecurityThreats1 WHERE [ИдентификаторУБИ]=@ИдентификаторУБИ", sqlConnection);
                    ...
                    else
                    {
                        ...
                        EXCEPTION2.Content = "Такого и  дентификатора УБИ не найдено!!!";
                    }
        ...
        private async void LoadedFiles()
        {
        ...
        private async void Button_Click_4(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                FileStream file = new FileStream(@"C:\Users\User\Desktop\table.txt", FileMode.Append);
        ...
        {
            SqlCommand command = new SqlCommand("SELECT * FROM InformationSecurityThreats1", sqlConnection);
            SqlDataReader sqlReader = sqlReader = await command.ExecuteReaderAsync();
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.HasValue)
            {
                try
                {
                    ...
                    {
                        ExcelWorksheet excelWorkSheet = package.Workbook.Worksheets.Add("InformationSecurityThreats1");

                        await excelWorkSheet.Cells.LoadFromDataReaderAsync(sqlReader, true);
        ...
        {
            MessageBoxResult mBox = MessageBox.Show("Вы уверены, что хотите выйти?", "Exit", MessageBoxButton.YesNo);
            if (MessageBoxResult.Yes == mBox)
            {
                Environment.Exit(0);
            }
